...
// 测试配置：从环境变量读取
// Embedding 使用 LLM_BASE_URL/LLM_API_KEY，模型单独设置
// 设置方式：
...
// getTestIntents 返回增强版测试意图配置（覆盖多样表达）
...
// runLocalIntentTest 封装单条测试的执行逻辑
func runLocalIntentTest(t *testing.T, node *LocalIntentNode, input string, expectedIntents []string) (resultRelation string, matched bool) {
	...
	case <-done:
		...
		if !matched {
			t.Errorf("期望路由到 %v，实际路由到 %s", expectedIntents, resultRelation)
		}
...
func TestLocalIntentNode_Init(t *testing.T) {
	...
	t.Run("无效的输入表达式", func(t *testing.T) {
		...
		err := node.Init(config, configuration)
		if err != nil {
			t.Logf("Init 返回错误: %v (该表达式可能被解析器接受，错误来自后续步骤)", err)
		} else {
			t.Log("该表达式被解析器接受了")
		}
	})
...
// runLocalIntentTestSilent 静默执行单条测试，不输出 t.Logf，返回结果
func runLocalIntentTestSilent(t *testing.T, node *LocalIntentNode, input string) (resultRelation string, matched bool) {
	...
	case <-done:
		if resultErr != nil {
			t.Logf("  输入 %q 错误: %v", input, resultErr)
		}
		matched = (resultRelation != "")
	case <-time.After(30 * time.Second):
		t.Errorf("输入 %q 超时", input)
	}
	return
}
